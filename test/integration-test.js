...
const Sqlite3LevelController = require('../lib/level-controllers/sqlite3-level-controller');
...
beforeEach(function () {
  ...
  elevator = new ElevatorBase(new ConsoleLogger(true), Sqlite3LevelController, process.cwd());

  fs.mkdirSync(tempTestFilesFolder)
  fs.copyFileSync(path.join(__dirname, 'resources', 'test'), path.join(tempTestFilesFolder, 'test'))
});
